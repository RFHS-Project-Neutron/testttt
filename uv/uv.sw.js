		...
		async fetch({ request: i }) {
			...
			try {
				...
				const m = await this.bareClient.fetch(a, {
						...
						mode:
							location.origin !== d.address.origin
								? 'cors'
								: d.mode,
						cache: d.cache,
						redirect: d.redirect
					...
					f = new s(d, m),
					b = new n(f, null, null);
				if ((this.emit('beforemod', b), b.intercepted))
				...
				if (
					...
					if (!/\s*?((inline|attachment);\s*?)filename=/i.test(e)) {
						const t = /^\s*?attachment/i.test(e)
								? 'attachment'
								: 'inline',
							[r] = new URL(m.finalURL).pathname
								.split('/')
								.slice(-1);
						...
						case 'document':
							(function (t, r = '') {
								return (
									'text/html' ===
									(
										e.mime.contentType(r || t.pathname) ||
										'text/html'
									).split(';')[0]
								);
				...
				return (
					...
					crossOriginIsolated &&
						(f.headers['Cross-Origin-Embedder-Policy'] =
							'require-corp'),
					...
					? (console.error(e),
						(function (e, t, r) {
							...
							!(function (e) {
								...
								: ((i = e.status),
									(s =
										'Error communicating with the Bare server'),
									(n = e.body.message),
									(o = e.body.code),
									(a = e.body.id));
							return new Response(
								(function (e, t, r, i, s, o, n) {
									...
									const a =
										...
										(r
											? `errorId.textContent = ${JSON.stringify(r)};`
											: '') +
									...
									return (
										...
										(r
											? '<tr><td>ID:</td><td id="errorId"></td></tr>'
											: '') +
